...
fn main() {
                ...
                tauri::async_runtime::spawn(async move {
                    while let Some(msg) = msg_rx.recv().await {
                        ...
                        let engine = match engine_guard.as_mut() {
                            ...
                            None => {
                                ...
                                let _ = im_manager_ref.message_router.send_reply(ReplyPayload {
                                    ...
                                    content: "�?AI 引擎未初始化，请先配置模�?.to_string(),
                        ...
                        let card_start = serde_json::json!({
                            ...
                            "elements": [
                                { "tag": "markdown", "content": "🤔 *正在思�?..*" },
                                { "tag": "note", "elements": [{ "tag": "plain_text", "content": "流式输出�?.." }] }
                            ]
                        });
                                    ...
                                    match rx.recv().await {
                                        ...
                                        Some(crate::native_engine::tool_loop::EngineEvent::MessageStop { .. }) => {
                                            ...
                                            let card_final = serde_json::json!({
                                                ...
                                                "header": { "title": { "tag": "plain_text", "content": "Claude" }, "template": "blue" },
                                                "elements": [
                                                    { "tag": "markdown", "content": &if full_text.is_empty() { "_(空回�?_" } else { &full_text } }
                                                ]
                                            });
    ...
    #[cfg(mobile)]
    {
        builder = builder
            ...
            .plugin(digital_life_tauri::init())
